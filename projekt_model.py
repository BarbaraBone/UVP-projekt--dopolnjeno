...
def slovarja_daljic(sez):
    ...
    for funkcija in funkcije:
        for abcisa in a:
            if float(funkcija.subs(x,abcisa)) == float(b[a.index(abcisa)]):
...
def vrednost_v_tocki(sez, x0):
    ...
    for daljica in funkcije: 
        if float(premica_zacetek[daljica]) <= float(x0) <= float(premica_konec[daljica]):
        ...
        if float (premica_konec[daljica]) <= float (x0) <= float (premica_zacetek[daljica]):
            if [x0, daljica.subs(x, x0)] not in  vrednosti_brez_funkcij:
                vrednosti.append([x0, daljica.subs(x, x0), daljica])
                vrednosti_brez_funkcij.append([x0, daljica.subs(x, x0)])
    return vrednosti
...
def nov_lik(sez, x0):
    ...
    j = 0
    #prvi del
    while float (premice_konec[zacetek_premice[sez[j][0]]]) < float (A[0]):
    ...
    #drugi del
    for tocka in sez[j:]:
        if float(tocka[0]) < float(B[0]) and tocka not in nov_lik:
...
def naredi_points(sez):
    ...
    points = []
    for tocka in sez:
        points.append((float (tocka[0]), float (tocka[1])))
    return points
...
def pretvorba_str_v_list(string):
    sez = []
    sez1 = list(string.split("],["))
    for element in sez1:
        element = element.replace("[","").replace("]","")
        sez2 = list(element.split(","))
        sez.append([float(sez2[0]),float(sez2[1])])
    return sez
...
class Ploscina:
    ...
    def ploscina_novega_lika(self):
        if float (self.meja) <= float (min(self.abcise)):
            return 0
        elif float(self.meja) >= (max(self.abcise)):
            return self.ploscina
        else:
